...
Two-stage flash-decoding that reads packed b-bit quantized KV buffers directly:
  - Stage 1: MSE codebook scoring for K, online softmax, MSE weighted sum for V.
  - Stage 2: Merge across KV splits (reuses SGLang's existing _fwd_kernel_stage2).

Both K and V use MSE-only quantization (Algorithm 1). This aligns with
community consensus that QJL (Algorithm 2) increases variance without
...
def _turboquant_fwd_kernel_stage1(
    # Pre-rotated queries: [B, H_q, D]
    Q_rot,
    # Packed K buffers (MSE-only)
    ...
    stride_k_mse_bs,
    stride_k_mse_h,
    # Strides — K_Norms: [pool_size, H_kv]
    stride_kn_bs,
    ...
    MSE_BITS: tl.constexpr,       # key MSE quantization bits
    ...
    """TurboQuant decode attention Stage 1 (MSE-only keys).

    For each KV split assigned to this program:
    1. Score keys via MSE codebook lookup (q_rot @ codebook[idx] * norm)
    2. Online softmax
    3. Weighted-sum values via MSE codebook gather in rotated space
    ...
    if split_kv_end > split_kv_start:
        ...
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            ...
            # ==============================================================
            # K scoring: MSE codebook lookup
            # score = ||k|| * q_rot^T @ C[idx] / sqrt(d)
            # ==============================================================
            ...
            # Score = ||k|| * mse_score * sm_scale
            k_norms = tl.load(
                K_Norms + kv_loc * stride_kn_bs + cur_kv_head,
                mask=n_mask, other=0.0,
            ).to(tl.float32)

            qk = k_norms * mse_score * sm_scale
            ...
            # ==============================================================
            # V weighted sum: TurboQuant_mse (paper Algorithm 1)
...
def turboquant_decode_attention_fwd(
    ...
    mse_bits: int,                  # MSE quantization bits for keys
    ...
    """TurboQuant fused decode attention (two-stage flash-decoding, MSE-only keys).

    Stage 1: For each KV split, score keys via MSE codebook lookup,
             apply online softmax, and accumulate MSE V weighted sum.
...
def _turboquant_fwd_kernel_stage1_split(
    # Pre-rotated queries per group
    Q_rot_lo,           # [B, H_q, D_lo]
    # Packed K buffers — lo group (MSE-only)
    K_MSE_Buffer_lo,    # [pool_size, H_kv, packed_mse_width_lo] uint8
    K_Norms_lo,         # [pool_size, H_kv] float16
    # Pre-rotated queries — hi group
    Q_rot_hi,           # [B, H_q, D_hi]
    # Packed K buffers — hi group (MSE-only)
    ...
    stride_k_mse_lo_bs,
    stride_k_mse_lo_h,
    # Strides — K_Norms_lo: [pool_size, H_kv]
    stride_kn_lo_bs,
    # Strides — K_MSE_hi: [pool_size, H_kv, packed_mse_width_hi]
    stride_k_mse_hi_bs,
    stride_k_mse_hi_h,
    # Strides — K_Norms_hi: [pool_size, H_kv]
    stride_kn_hi_bs,
    ...
    if split_kv_end > split_kv_start:
        ...
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            ...
            # ==============================================================
            # K scoring — lo group: MSE codebook lookup
            # ==============================================================
            ...
            # ==============================================================
            # K scoring — hi group: MSE codebook lookup
            # ==============================================================
            ...
            qk = (k_norms_lo * mse_score_lo + k_norms_hi * mse_score_hi) * sm_scale
            qk = tl.where(n_mask, qk, float("-inf"))
...
def turboquant_decode_attention_fwd_split(
    ...
    k_packed_lo: torch.Tensor,      # [pool_size, H_kv, packed_width_lo] uint8
    k_norms_lo: torch.Tensor,       # [pool_size, H_kv] float16
    k_packed_hi: torch.Tensor,      # [pool_size, H_kv, packed_width_hi] uint8
    ...
    k_cb_lo: torch.Tensor,          # [2^k_bits_lo] float32
    k_cb_hi: torch.Tensor,          # [2^k_bits_hi] float32
    ...
    """TurboQuant split-channel fused decode attention (MSE-only keys).
    ...
    kv_head_num = k_packed_lo.shape[1]
    kv_group_num = head_num // kv_head_num
    ...
    # Stage 1: split-channel MSE scoring + softmax + V accumulation
    _turboquant_fwd_kernel_stage1_split[grid](
        ...
        q_rot_hi.stride(1),
        # K_lo strides
        k_packed_lo.stride(0),
        k_packed_lo.stride(1),
        # K_Norms_lo stride
        k_norms_lo.stride(0),
        # K_hi strides
        k_packed_hi.stride(0),
        k_packed_hi.stride(1),
        # K_Norms_hi stride
        k_norms_hi.stride(0),
        # V_Packed_lo strides
        ...
        # Constexprs
        kv_group_num=kv_group_num,
        MSE_BITS_LO=k_bits_lo,
        MSE_BITS_HI=k_bits_hi,
        V_BITS_LO=v_bits_lo,
        V_BITS_HI=v_bits_hi,
        D_LO=d_lo,
        D_HI=d_hi,
